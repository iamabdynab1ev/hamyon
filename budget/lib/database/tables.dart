...
class FinanceDatabase extends _$FinanceDatabase {
  ...
  Future<int?>? createOrUpdateTransaction(
    ...
    // После синхронизации на устройстве владельца оказываются операции всей
    // семьи, и различить их можно только по автору. Проставляем его один раз при
    // создании и не переписываем при правках, иначе запись присвоил бы себе тот,
    // кто последним открыл операцию.
    if (FamilyServer.isSignedIn &&
        (transaction.transactionOwnerEmail ?? "").isEmpty) {
      transaction = transaction.copyWith(
          transactionOwnerEmail: Value(FamilyServer.currentUserLogin));
    }
  ...
  // Проведённые операции за период. Сводка владельца считает итоги по людям и
  // категориям уже в приложении: после синхронизации все операции семьи и так
  // лежат в этой базе, поэтому отдельных запросов на сервер не требуется.
  Future<List<Transaction>> getPaidTransactionsInRange(
      DateTime start, DateTime end) {
    return (select(transactions)
          ..where((tbl) =>
              tbl.dateCreated.isBiggerOrEqualValue(start.justDay()) &
              tbl.dateCreated.isSmallerOrEqualValue(end) &
              tbl.paid.equals(true))
