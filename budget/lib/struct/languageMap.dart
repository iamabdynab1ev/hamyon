...
// Flutter's built-in localizations (date pickers, "OK"/"Cancel", tooltips) cover
// fewer languages than we ship app translations for - Tajik is not among them.
// These delegates serve Russian system strings for any locale Flutter does not
// know, so the app strings stay translated while the framework still finds a
// MaterialLocalizations. Localizations only uses the first delegate of each type
// that supports the locale, so these must be listed after the global delegates.
const Locale fallbackSystemStringsLocale = Locale("ru");
...
class FallbackMaterialLocalizationsDelegate
  ...
  Future<MaterialLocalizations> load(Locale locale) =>
      GlobalMaterialLocalizations.delegate.load(fallbackSystemStringsLocale);
...
class FallbackCupertinoLocalizationsDelegate
  ...
  Future<CupertinoLocalizations> load(Locale locale) =>
      GlobalCupertinoLocalizations.delegate.load(fallbackSystemStringsLocale);
...
class FallbackWidgetsLocalizationsDelegate
  ...
  Future<WidgetsLocalizations> load(Locale locale) =>
      GlobalWidgetsLocalizations.delegate.load(fallbackSystemStringsLocale);

  @override
  bool shouldReload(FallbackWidgetsLocalizationsDelegate old) => false;
}

List<LocalizationsDelegate> localizationDelegatesWithFallback(
