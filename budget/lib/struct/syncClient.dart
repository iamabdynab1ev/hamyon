...
import 'package:hamyon/widgets/util/debouncer.dart';
...
Future<bool> createSyncBackup(
  ...
  // Со своим сервером семьи вход в Google не нужен, поэтому его проверки
  // применяются только когда файлы лежат в Drive.
  if (isUsingFamilyServer == false) {
  ...
  if (isUsingFamilyServer == false) {
    ...
    // В Drive каждая выгрузка создаёт новый файл, поэтому предыдущий файл этого
    // устройства нужно убрать. На своём сервере выгрузка его перезаписывает.
    for (RemoteBackupFile file in await listRemoteBackupFiles()) {
      if (isCurrentDeviceSyncBackupFile(file.name)) {
        try {
          await deleteRemoteBackupFile(file);
        } catch (e) {
          print(e.toString());
        }
      }
...
Future<bool> _syncData(BuildContext context) async {
  ...
  List<RemoteBackupFile> filesToDownloadSyncChanges = [
    for (RemoteBackupFile file in await listRemoteBackupFiles())
      if (isSyncBackupFile(file.name)) file
  ];

  print("LOADING SYNC DB");
  DateTime syncStarted = DateTime.now();
  List<SyncLog> syncLogs = [];
  List<RemoteBackupFile> filesSyncing = [];

  int currentFileIndex = 0;
  loadingProgressKey.currentState?.setProgressPercentage(0);
  for (RemoteBackupFile file in filesToDownloadSyncChanges) {
    ...
    print(lastSynced);
    if (lastSynced.isAfter(file.modifiedTime) ||
  ...
  for (RemoteBackupFile file in filesSyncing)
    setDateOfLastSyncedWithClient(
        getDeviceFromSyncBackupFileName(file.name), file.modifiedTime);
